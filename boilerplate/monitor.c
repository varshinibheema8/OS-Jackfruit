...
/* Kernel 6.15+ renamed del_timer_sync -> timer_delete_sync */
 ...
 *   - The timer callback runs in softirq context but with
 *     del_timer_sync we ensure it's done before unload.
 *   - Our timer callback uses kzalloc/kfree which may sleep,
 *     so a spinlock would be unsafe there.
 *   - ioctl paths are process context, so mutex is fine.
 * ============================================================== */
...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        if (!entry->soft_warned && rss > (long)entry->soft_limit_bytes) {
